...
See [`SYSTEM_DESIGN.md`](./SYSTEM_DESIGN.md) for the architecture write-up (double-booking prevention, leave conflicts, notification/calendar failure handling, latency).
...
3. Add `http://localhost:8000/api/calendar/callback` (or your deployed URL) as an authorized redirect URI.
4. Put the client ID/secret in `.env`.
5. Users connect via `POST /api/calendar/connect`, which returns a consent URL; Google redirects back to `/api/calendar/callback`.
